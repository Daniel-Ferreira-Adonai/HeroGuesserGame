...
function Card({hero, heroOfTheDay}) {
  ...
  return (
    <div className="custom_card">
      ...
      <div className={"background-answers " + hero.popIn} style={{background: decideColorString(hero.name, heroOfTheDay.name)}}>
        <h3>{hero.name ?? "not found"}</h3>
      </div>

      <div className={"background-answers " + hero.popIn} style={{background: decideColorString(hero.gender, heroOfTheDay.gender)}}>
        <h3>{hero.gender ?? "not found"}</h3>
      </div>

      <div className={"background-answers " + hero.popIn} style={{background: decideColorArray(hero.powers, heroOfTheDay.powers)}}>
        <h3>{hero.powers ? hero.powers.map((x,i) => i === hero.powers.length - 1 ? x : x + ", ") : "not found"}</h3>
      </div>

      <div className={"background-answers " + hero.popIn} style={{background: decideColorString(hero.species, heroOfTheDay.species)}}>
        
        <h3>{hero.species ?? "not found"}</h3>
      </div>

      <div className={"background-answers " + hero.popIn} style={{background: decideColor(hero.publisher, heroOfTheDay.publisher)}}>
        <h3 style={{zIndex: 3}}>{hero.publisher ?? "not found"}</h3>
      </div>

      <div className={"background-answers " + hero.popIn} style={{background: decideColor(hero.date, heroOfTheDay.date)}}>
      ...
      <div className={"background-answers " + hero.popIn} style={{background: decideColor(hero.height, heroOfTheDay.height)}}>
